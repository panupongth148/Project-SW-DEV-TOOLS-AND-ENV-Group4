...
describe('Compont store test', async () => {


        it('It should an array and status 200', () => {
            // chai.spy.on(server, ["get"])
            chai.request(server)
            .get("/store/managebook/jxxgfXADlI60DGxGpw1D")
                .end((err, response) => {
        ...
        it('It should be return 200 OK', () => {
            chai.request(server).delete("/store/deleteBook/842mQclFUDxWfc3JOivp")
                .end((err, response) => {
                    // console.log(response.json())
                    expect(response).to.have.status(200);
                    // JSON.parse(response.text).equal('success');
                    
                })
        })

        it('It should be return 200 OK', () => {
            chai.request(server).put(`/store/editbook/842mQclFUDxWfc3JOivp`)
